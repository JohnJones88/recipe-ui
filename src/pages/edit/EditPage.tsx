...
function EditPage() {
    ...
    const [step, setStep] = useState([{ sortNumber: '', recDescription: '' }]);
    ...
    useEffect(() => {
        ...
        const fetchRecipe = async () => {
            ...
            const options = {
                method: 'GET',
                headers: { 'authorization': `${localStorage.getItem('profile-token')}` },
            };
            try {
                ...
                setStep(data.step); // Assuming the fetched data contains the steps
            } catch (error) {
                console.error(error);
            }
        };
    ...
    const uploadImage = async (rawImage: File | null) => {
   ...
   const updateRecipe = async () => {
    ...
    const options = {
        ...
        body: JSON.stringify({ name, food_Type, description, imageUrl, step })
    };
    try {
        const response = await fetch(url, options);
        if (response.status === 401) {
            ...
            const data = await response.json();
            console.log(data);
            // Navigate to the ViewPage after successful update
            navigate(`/view/${data.id}`); // Assuming data.id contains the updated recipe's ID
        }
    } catch (error) {
        console.error(error);
    }
    ...
    return (
        <div className="container pt-6">
            ...
            <div>
                <button type="button" className="btn btn-primary" onClick={updateRecipe}>Update</button>
            </div>
